...
const AdminAddPetScreen = ({ onGoBack, adminToken }) => {
  ...
  const searchLocation = async (query) => {
    ...
    try {
      const apiKey = process.env.EXPO_PUBLIC_GOOGLE_MAPS_API_KEY || ''; // Ensure this key is set in .env
      const url = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(query)}&components=country:PH&key=${apiKey}`;
      ...
      if (data.status === 'OK') {
        const formattedResults = data.results.map((item, index) => ({
          place_id: item.place_id || String(index),
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        Alert.alert('Error', 'Failed to add pet to Firebase.');
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalHint}>
              <Ionicons name="information-circle" size={14} color={ADMIN_COLORS.textMuted} /> Powered by Google Maps
            </Text>
