...
const AdminPetsScreen = ({ onGoBack, onNavigate, adminToken }) => {
  ...
  const fetchPetCategories = useCallback(async () => {
    try {
      const dbCategories = await getDocs(collection(db, 'pet_categories'));
      if (!dbCategories.empty) {
        setPetCategories(dbCategories.docs.map(doc => ({ id: doc.id, name: doc.data().name })));
      }
    } catch (error) {
  ...
  const fetchPets = useCallback(async () => {
    try {
      ...
      const dbPets = petsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      
      let result = dbPets;

      // Note: Admin filtering gets applied via frontend useMemo anyway,
      // but if we were applying it here we'd filter similarly.
      setPets(result);
