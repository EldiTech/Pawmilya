...
import { shelterService, userService } from '../../services';
import { getAvatarUrl } from '../../utils/userShared';
...
const TWO_FACTOR_STORAGE_KEY = 'pawmilya_two_factor_enabled';
...
const UserSettingsScreen = ({ onLogout, onNavigateToRescuerRegistration, onNavigateToAdoptions, onOpenFeedback, onOpenReportProblem, onNavigateToJemoy, onNavigateToShelterApplication, onNavigateToShelterManager, onNavigateToNotifications }) => {
  ...
  useEffect(() => {
    const loadStored2FA = async () => {
      try {
        const storedValue = await AsyncStorage.getItem(TWO_FACTOR_STORAGE_KEY);
        if (storedValue === 'true' || storedValue === 'false') {
          setTwoFactorEnabled(storedValue === 'true');
        }
      } catch (error) {
        // Keep current state if storage is unavailable
      }
    };
  ...
  useEffect(() => {
    const checkManagerStatus = async () => {
      try {
        ...
        setIsShelterManager(result?.success ? result.isManager : false);
      } catch (e) {
  ...
  const settingsSections = useMemo(() => {
    return SETTINGS_SECTIONS.map(section => {
      if (section.title === 'Activity') {
        const isRescuer = user?.is_rescuer === true;
        const rescuerStatus = user?.rescuer_status;
        ...
        return {
          ...
          items: section.items.map(item => {
            ...
            if (item.id === 'becomeRescuer') {
              if (isRescuer && rescuerStatus === 'approved') {
                return { ...item, icon: 'medkit', label: 'Rescuer Dashboard' };
              } else if (rescuerStatus === 'pending') {
                return { ...item, icon: 'time', label: 'Rescuer Pending' };
              } else if (rescuerStatus === 'rejected') {
                return { ...item, icon: 'alert-circle', label: 'Rescuer Application Rejected' };
              }
              return item;
            }
            return item;
  ...
  }, [isShelterManager, user?.is_rescuer, user?.rescuer_status]);
  ...
  const fetchUserProfile = useCallback(async () => {
    try {
      ...
      const data = profileResponse?.success ? profileResponse?.data : null;
      if (data) {
  ...
  const handleToggle2FA = useCallback(async () => {
    ...
    try {
      await AsyncStorage.setItem(TWO_FACTOR_STORAGE_KEY, String(nextValue));
    } catch (error) {
    ...
    try {
      if (typeof userService?.toggle2FA === 'function') {
        ...
        if (typeof enabled === 'boolean') {
          ...
          try {
            await AsyncStorage.setItem(TWO_FACTOR_STORAGE_KEY, String(enabled));
          } catch (storageError) {
      ...
      // Keep local toggle state when backend sync fails.
    }
  ...
  const handleDeleteAccount = useCallback(async () => {
    try {
      ...
      if (!response?.success) {
        throw new Error(response?.error || 'Failed to delete account. Please try again.');
      }
    ...
    if (item.id === 'profile') {
      ...
      if (isShelterManager) {
        if (onNavigateToShelterManager) {
          ...
          Alert.alert('Coming Soon', 'Shelter Manager dashboard is currently in development! Stay tuned!');
        }
      } else {
        ...
        if (tempAvatar.startsWith('http://') || tempAvatar.startsWith('https://')) {
          ...
          if (typeof userService?.uploadAvatar === 'function') {
            ...
            try {
              ...
              // Keep local URI fallback when upload endpoint is unavailable.
              newAvatarUrl = tempAvatar;
            } finally {
  ...
  const renderSettingItem = useCallback((item) => {
    const isToggle2FA = item.type === 'toggle' && item.id === 'twoFactor';
    const isLinkAction = item.type === 'link';
  ...
  }, [handleToggle2FA, twoFactorEnabled, handleSettingPress]);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        >
          ...
          >
            <View style={styles.jemoyIconWrap}>
              <Image source={require('../../assets/images/jemoy.png')} style={{ width: 44, height: 44, borderRadius: 22 }} resizeMode="cover" />
            </View>
        ...
        <TouchableOpacity style={styles.logoutButton} onPress={() => setSwitchAccountModalVisible(true)} activeOpacity={0.8}>
          <Ionicons name="people-outline" size={22} color={COLORS.primary} />
          <Text style={[styles.logoutText, { color: COLORS.primary }]}>Switch Account</Text>
        </TouchableOpacity>
        ...
        <View style={styles.mainBottomSpacing} />
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  mainBottomSpacing: {
    height: Platform.OS === 'ios' ? 132 : 112,
  },
