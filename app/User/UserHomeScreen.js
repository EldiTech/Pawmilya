...
import { petService, shelterService, userService } from '../../services';
...
import {
    ...
    handleApiError,
    parseApiResponse,
} from '../../utils/userShared';

const UserHomeScreen = ({ onNavigateToRescue, onNavigateToPets, onNavigateToNotifications, onNavigateToReportRescue, onNavigateToAdoptions, onNavigateToRescuerRegistration, onNavigateToShelterManager, activeTab, notificationRefreshKey = 0 }) => {
  const { user } = useAuth();
  const userIdentityKey = user?.uid || user?.id || user?.email || null;
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      try {
        const shelterRes = await shelterService.getShelterManagerStatus();
        setIsShelterManager(shelterRes?.success ? shelterRes.isManager : false);
      } catch (e) {
  ...
  }, [userIdentityKey, fetchData]);

  // Sync name/avatar when user context changes (e.g. settings save)
  useEffect(() => {
    setUserData((prev) => ({
      ...prev,
      name: user?.full_name || user?.displayName || user?.name || prev.name || 'User',
      avatar: user?.avatar_url || user?.photoURL || prev.avatar,
    }));
  }, [user?.full_name, user?.displayName, user?.name, user?.avatar_url, user?.photoURL]);

  // Refresh data when activeTab changes to 'home'
  useEffect(() => {
    if (activeTab === 'home') {
      fetchData();
      fetchNotificationCount();
    }
  }, [activeTab, fetchData, fetchNotificationCount]);
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity style={styles.notificationBtn} onPress={onNavigateToNotifications} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
        ...
        <View style={styles.mainActionsSection}>
          ...
          {(user?.is_rescuer === true && user?.rescuer_status === 'approved') && (
            <TouchableOpacity style={[styles.secondaryActionCard, { marginTop: SPACING.md }]} activeOpacity={0.8} onPress={onNavigateToRescuerRegistration}>
              <View style={styles.actionCardContent}>
                <View style={styles.actionCardLeft}>
                  <View style={[styles.actionCardIconContainer, { backgroundColor: COLORS.info + '15' }]}>
                    <Ionicons name="medkit" size={32} color={COLORS.info} />
                  </View>
                  <View style={styles.actionCardText}>
                    <Text style={styles.secondaryActionCardTitle}>Rescuer Dashboard</Text>
                    <Text style={styles.secondaryActionCardSubtitle}>Manage your rescue operations</Text>
                  </View>
                </View>
                <Ionicons name="chevron-forward" size={24} color={COLORS.textDark} />
              </View>
          ...
          {isShelterManager && (
            <TouchableOpacity style={[styles.secondaryActionCard, { marginTop: SPACING.md }]} activeOpacity={0.8} onPress={onNavigateToShelterManager}>
              <View style={styles.actionCardContent}>
                <View style={styles.actionCardLeft}>
                  <View style={[styles.actionCardIconContainer, { backgroundColor: COLORS.success + '15' }]}>
                    <Ionicons name="home" size={32} color={COLORS.success} />
                  </View>
                  <View style={styles.actionCardText}>
                    <Text style={styles.secondaryActionCardTitle}>Shelter Dashboard</Text>
                    <Text style={styles.secondaryActionCardSubtitle}>Manage your shelter operations</Text>
                  </View>
                </View>
                <Ionicons name="chevron-forward" size={24} color={COLORS.textDark} />
              </View>
